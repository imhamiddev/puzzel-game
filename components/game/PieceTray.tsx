...
export default function PieceTray({ pieces, pieceSize }: PieceTrayProps) {
  ...
  return (
    ...
    >
      <div className="flex gap-2.5 overflow-x-auto pb-1 no-select" style={{ minHeight: pieceSize }}>
        {pieces.length === 0 && (
          <p className="text-white/30 text-sm py-3 px-1">All pieces placed on the board</p>
        )}
        {pieces.map((piece) => (
